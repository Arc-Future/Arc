...
浏览器引擎需要附着到原生窗口。Arc 通过 `WindowHost` 提供原生窗口句柄提取能力——runtime 层窗口后端按平台实现（下表为各平台句柄映射）。**0.1 已验证并随包交付的面为 Windows**；Linux（X11）/macOS（AppKit）后端已接线但未随 0.1 验收（UI/vendor 面边界见 [11 编译模型](11-compilation-model.md)），示例请以 Windows 为准。
