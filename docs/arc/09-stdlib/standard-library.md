...
本页面向**应用开发者**：告诉你有哪些命名空间、怎么引用、诚实能力边界在哪。实现级 ABI / Provider 全家桶规划见仓库 RFC，不在此展开。

## 核心库 vs 领域库

| 层级 | 目录 / 命名空间 | 说明 |
|------|-----------------|-------------|
| **核心** | `std/Arc/`：Collections、IO、Text、Tasks、Exceptions、Console/Math/基元、LINQ **Enumerable** 等 | 日常应用的默认底座；须有可验证行为 |
| **领域** | `std/Orm/`、`std/UI/Core/`、`std/Net/`、`std/DI/`、`std/Web/` 等 | 按需依赖；各子库独立演进。未实现的面须显式失败（如 `NotImplementedException`），禁止空实现冒充完备 |

**Queryable SQL / Orm Provider** 属领域翻译：编译器核心不嵌领域能力。未实现的 Provider 面必须诚实报错；**禁**假开「全家桶已交付」口径。
...
| `Array` | `Arc` | `Copy`/`Clear`/`Reverse`；`int[]` 的 `IndexOf`/`LastIndexOf`/`Resize`；见 [12 运行时 ABI](../12-advanced/design-decisions.md) |
...
模块解析（import/using）见 [04 词法与语法](../03-syntax/lexicon-syntax.md)。
...
std 通过 `rt_*` ABI 调用运行时；平台相关能力在 `crates/runtime/platform/` 与 capability 系统中声明（见[能力系统](../12-advanced/capabilities.md)）。

## Arc.Orm 框架构建指南（L3 · 骨架）

Arc.Orm 是基于 Arc 语言核心能力构建的 ORM 框架层。本节说明如何利用[编译期树化 + 运行时翻译](../08-query/query-language.md)范式构建数据库 Provider。
...
对标 WPF：先按族名注册字体文件，再在 ARML 里写 `FontFamily`。设计权威见 [RFC 037 §9](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/037-ui.md) / [custom-fonts](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/037-ui/references/custom-fonts.md)。**不宣称完备**（无 pack URI、无 `FontStyle`、无 HarfBuzz/彩色 emoji）。
...
| **QIF 验证底座** | Equal（含 bool/long/Δ）、Same/NotSame、StartsWith/EndsWith、Contains·DoesNotContain、All/Any/Single(+pred)/SequenceEqual；`[Fact(Timeout)]` 覆盖套件默认；并行：`--parallel` 集间并行/集内串行（xUnit 子集；async 强制串行）；fixture 诚实等价：`IQIFOutput`+Setup/Teardown（非完整 IClassFixture）；诚实边界：**仅 L1 稳定**，不提供 L2–L7 全家桶；`--coverage` 未落地；泛型 `Throws<T>`/`IsType<T>(object)` 延后 |
