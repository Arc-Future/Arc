...
**编译器职责止于树化**：`crates/ast`（`expr_tree.rs`）构建 `ExpressionIr`，`typeck` 将 `Expression<T>` Lambda 树化为 IR，`codegen` 生成运行时构造 `Expression` 对象树的代码（见[表达式树](expression-trees.md)）。Provider **实现**在**运行时**遍历 `Expression` 对象树并翻译为存储或远程执行计划。
