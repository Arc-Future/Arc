...
**编码与命名对照**（C# 采纳 / Rust 拒绝）见 [03 编码与语法标准](encoding-standard.md)。
...
`record` / `record struct` 与后缀 `with { … }`、位置 `Deconstruct` 见 [07 对象模型](../06-objects/object-model.md)。解构赋值 `(x, y) = expr`、弃元 `_`、声明式 `var (x, y) = expr`、位置模式 `is`/`switch`（含 switch 表达式）见 [07 对象模型](../06-objects/object-model.md)。`init` 访问器见 [07 对象模型](../06-objects/object-model.md)。
...
var result = compute()?;   // 错误传播，见[类型系统](../04-types/type-system.md)
```
...
等价于 LINQ 方法链（见[查询语言](../08-query/query-language.md)）。
...
详见[异步与任务](../07-async/async-tasks.md)。
