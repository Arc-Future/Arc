...
> **Transient disposable 警示**：transient 实例不由容器跟踪，实现 `IDisposable` 时由解析方负责释放（对齐 .NET MEDI；权威见 [RFC 023](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/023-math-tensor-di.md)）。
...
- **服务注册入口**：`[Inject]` 消费方经显式注册调用 / 静态构造器装配（源码打包合并单一编译单元），见规范章 [RFC 037 §6](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/037-ui.md)。
- **设计决策与产品化冲刺路线**：权威出处为 [RFC 023](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/023-math-tensor-di.md)（含设计决策与明确不做项）。
