...
| 审视/反馈 | 离屏回读 `RenderCapture`（[ai-native-render-capture](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/037-ui/references/ai-native-render-capture.md)）+ [fidelity-loop](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/037-ui/references/ai-native-fidelity-loop.md)：headless 可测，供 LLM 多模态校验，闭环自动化（与 [RFC 034 AI 原生工具链](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/034-ai-toolchain-arcgr.md) 同构） |
...
**已存在、直接复用：**
- 离屏 target + 纹理合成通道（`IRender.CreateOffscreenTarget/RenderToOffscreen/ReadbackPixels`，见 [ai-native-render-capture](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/037-ui/references/ai-native-render-capture.md)）。
