...
> **平台现状（0.1）**：本章完整链路以 **Windows** 为交付与验证面（示例库 WebView2/CEF/Miniblink 均为 Windows 组件）。`.ani` 契约、编译期符号验证与动态库机制本身平台中立（命名规则 `.dll`/`.so`/`.dylib` 见 [12 运行时 ABI](../12-advanced/design-decisions.md)）；`WindowHost` 原生窗口句柄 API 在 runtime 层为三平台实现（映射见「原生窗口集成」），但 Linux/macOS 的 Arc.UI 渲染链与原生模块 vendor 未随 0.1 交付（wgpu 底座 M3+；平台能力边界见 [11 编译模型](../12-advanced/design-decisions.md)）——相关示例请在 Windows 上验证。
...
void Main() {
    ...
    // 输出: Loaded: browser-plugin, Version=0.1.0, Edition=1
...
浏览器引擎需要附着到原生窗口。Arc 通过 `WindowHost` 提供原生窗口句柄提取能力——runtime 层窗口后端按平台实现（下表为各平台句柄映射）。**0.1 已验证并随包交付的面为 Windows**；Linux（X11）/macOS（AppKit）后端已接线但未随 0.1 验收（UI/vendor 面边界见 [11 编译模型](../12-advanced/design-decisions.md)），示例请以 Windows 为准。
...
public void OnLoaded(AssemblyLoadArgs args) {
    ...
    if (meta.Version != "0.1.0") {
        throw new InvalidOperationException(
            "Version mismatch: expected 0.1.0, got " + meta.Version);
    }
}
```
...
| 主题 | 文档 |
|------|------|
| `arc.toml` 完整 schema | [17 arc.toml 项目清单参考](arc-toml-reference.md) |
| 能力系统 | [15 能力系统](../12-advanced/capabilities.md) |
