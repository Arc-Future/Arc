...
> `-r/--target` 目标三元组须为宿主桌面平台（win/linux/mac）；交叉编译管线未实现（现状见 [11 编译模型](../12-advanced/design-decisions.md)）。
...
对标 `dotnet test`。扫描源码 AST 收集 `[Fact]`/`[Theory]` 测试方法，自动生成测试宿主入口，编译为可执行文件并运行。`PROJECT` 可为 `.as` 文件或项目目录。默认**串行**（确定性优先）；并行为显式 opt-in，语义对标 xUnit 子集（集合间并行、集合内串行）。
...
| `--filter <EXPRESSION>` | XUnit 风格过滤（`Field~Value` + `\|`/`&`/`!`/括号） | `dotnet test --filter` |
...
| `--list-format <FMT>` | 列表格式：`text`（默认）\| `json` | — |
| `--logger <LOGGER>` | 输出格式：`human`（默认）/ `json` / `junit` | `dotnet test --logger` |
| `--parallel` | 集合级并行（集间并行、集内串行；async 套件强制串行） | xUnit `parallelizeTestCollections` |
| `--max-parallel <N>` | 最大并发度（仅配合 `--parallel`；`0`=不限） | — |
| `--timeout <MS>` | 默认单测超时毫秒（`0`=不限） | — |
| `--fail-on-skip` | Assert.Skip 计 Skipped 时非零退出（属性 Fact-Skip 始终硬失败） | — |
| `--coverage` | 覆盖率 → lcov（**尚未落地**，调用报错） | — |
| `-g`, `--debug` | 发射 DWARF 5 调试信息 | — |
| `--ani-native-lib <DIR>` | Native 库搜索路径（可重复） | — |
| `--obj-dir <DIR>` | 中间产物目录（覆盖 `obj/<config>/`） | — |
...
# 命名空间 / Kind / 并行（UnitTest 语料 QIF 自测命名空间为 UnitTest.QIF）
arc test examples/UnitTest --namespace UnitTest.QIF --kind Fact
arc test examples/UnitTest --parallel --max-parallel 4 --logger json

# 列出测试（不执行）
arc test examples/UnitTest --list-tests
arc test examples/UnitTest --list-tests --list-format json

# 跳过编译直接运行
arc test examples/UnitTest --no-build
```

并行与跳过语义权威见 [RFC 032](../../rfc/032-qif.md) §6 / §8。退出码：失败/错误/属性 Fact-Skip → 非零；`Assert.Skip` 默认 0，`--fail-on-skip` 时非零。
...
### `arc new <DIR> [--name <PKG>] [--agent] [--no-readme]`

生成最小可编译项目骨架（对标 `dotnet new console`）：`arc.toml` + `Program.as`（`void Main()`）+ 可选 `README.md`。`--agent` 追加 `Arc.Agent` / `Arc.Agent.Harness` 依赖并落 `.arcagent/conventions.md` 模板。

```bash
arc new ./MyApp
arc new ./AgentApp --agent
arc new ./Lib --name My.Lib --no-readme
```

### `arc detect [DIR] [--format human|json]`

识别目录项目类型（控制台 / ARML / Agent 等），供脚手架与智能体首触使用。
...
> `arc toolchain` / doctor clang 基线详见 [031 §11](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/031-compiler-cli.md)。

> `arc env` / `arc doctor` 定位规则与打包/安装脚本详见 [017 sdk-layout](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/017-build-artifacts-packages/references/sdk-layout.md) 与 [031 §10](https://github.com/Arc-Future/Arc/blob/main/docs/rfc/031-compiler-cli.md)。
...
| 码 | 含义 |
|----|------|
| 0 | 成功（`arc test`：无失败/错误/属性 Fact-Skip；Assert.Skip 默认仍计 0） |
| 1 | 诊断错误、IO/链接失败，或测试失败（含 Fact-Skip；`--fail-on-skip` 时含 Assert.Skip） |
