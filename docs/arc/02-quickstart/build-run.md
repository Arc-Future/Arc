...
> 前置：已安装 `arc` 且 clang/LLVM 可用（见 [安装与快速开始](getting-started.md)；编译器按解析序自动发现，无需手工配置）。
...
> `-r/--target` 指定目标三元组（桌面平台 win/linux/mac）；目标须为宿主桌面或具备完整工具链的环境——交叉编译管线未实现（现状见 [11 编译模型](../12-advanced/design-decisions.md)）。
...
- [编译器 CLI](../10-toolchain/compiler-cli.md) 查看 `arc` 全部子命令（含 `publish` / `test` / `inspect`）
- [arc.toml 项目清单](../10-toolchain/arc-toml-reference.md) 了解项目清单配置
