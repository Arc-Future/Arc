// RFC 038：Agnes 请求体 JSON 构造（internal，供 AgnesChatClient 门面调用）。
//
// OpenAI 兼容 chat/completions 请求体：model / messages（含 tool_calls 回显与
// reasoning_content 回传）/ tools / tool_choice / response_format / stream /
// stream_options / max_tokens / temperature / top_p / thinking / chat_template_kwargs /
// reasoning_effort。StringBuilder 拼接（线性复杂度）。
namespace Arc.Agent.Agnes;

using Arc.Agent;
using Arc.Text;

/// <summary>Agnes 请求体 JSON 构造器（StringBuilder 拼接，与门面解耦）。</summary>
internal static class AgnesRequestBuilder
{
    /// <summary>构造 chat/completions 请求体 JSON；stream 决定发射流式开关与 include_usage。</summary>
    public static string BuildRequestJson(AIRequest request, AgnesOptions options, bool stream)
    {
        StringBuilder sb = new StringBuilder();
        sb.Append("{");

        // model
        string model = options.Model != null ? options.Model : "agnes-2.0-flash";
        sb.Append("\"model\":\"");
        ...
        if (request != null && request.Messages != null)
        {
            ...
            for (int i = 0; i < msgCount; i++)
            {
                ...
                if (role == "assistant" && m.ToolCalls != null && m.ToolCalls.Count > 0)
                {
                    ...
                    // 推理模型工具调用轮次须完整回传 reasoning_content，否则思考链上下文断裂。
        ...
        // tool_choice（AgnesOptions 显式约束；空串不发射）
        if (options.ToolChoice != null && options.ToolChoice != "")
        {
            sb.Append(",\"tool_choice\":");
            sb.Append(options.ToolChoice);
        }

        // response_format（MAF contract-first：宿主声明契约，此处按 OpenAI 协议映射）
        ...
        if (stream)
        {
            // include_usage：流式末块携带 usage（业务端 token 统计 / cache 命中观察）。
        ...
        // thinking — enables reasoning output（可经 AgnesOptions.Thinking 关闭；-1 不发射）
        if (options.Thinking >= 0)
        {
            sb.Append(",\"thinking\":{\"type\":\"enabled\"}");
        }

        // chat_template_kwargs（Anthropic 兼容扩展；原始 JSON 透传，空串不发射）
        if (options.ChatTemplateKwargs != null && options.ChatTemplateKwargs != "")
        {
            sb.Append(",\"chat_template_kwargs\":");
            sb.Append(options.ChatTemplateKwargs);
        }
