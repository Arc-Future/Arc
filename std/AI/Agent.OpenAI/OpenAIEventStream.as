// RFC 038：OpenAI 流式事件序列（internal，供 OpenAIChatClient 门面构造）。
//
// 原生异步迭代器（RFC 044 单一惯用法）：冷启动建连 → SSE 拉取 → 领域映射 → 终结
// 收尾全部线性化在一个迭代器方法里——yield return 即事件投递，无手写状态机/待发
// 队列。每个 data: 行是完整 JSON 文档（OpenAIResponse DTO 类型化反序列化）；
...
/// <summary>
/// OpenAI 流式事件流生产：把 OpenAI 兼容 SSE 响应解码为 <see cref="AIStreamEvent"/>
/// 异步序列。HTTP 发送延迟到首次枚举（冷流）；取消经参数令牌全链路传播。
/// 异常经非抛错边界（StartAsync / ParseLine / SseDecoder.ReadAsync）收敛为值，
/// 流内错误一律转 Error 终结事件而非逃逸。
/// </summary>
internal static class OpenAIEventStream {

    /// <summary>流式事件序列：延迟建连 → 逐 SSE 事件领域映射 → 终结收尾。</summary>
    internal static async IAsyncEnumerable<AIStreamEvent> Events(OpenAIOptions options, HttpClient http, string url, AIRequest request, CancellationToken cancellationToken) {
        OpenAIStreamStart start = await OpenAIEventStream.StartAsync(options, http, url, request);
        if (start.Error != null) {
        ...
        while (true) {
            if (cancellationToken.IsCancellationRequested) {
                resp.Dispose();
                yield return AIStreamEvent.Error("Cancelled", "OpenAIChatClient.StreamEventsAsync: canceled during stream");
                yield break;
            }
            SseReadStep step = await SseDecoder.ReadAsync(sse);
            if (step.Error != null) {
                resp.Dispose();
                yield return AIStreamEvent.Error("StreamError", "OpenAIChatClient.StreamEventsAsync: " + step.Error);
                yield break;
            }
            if (!step.Moved) {
                break;
            ...
            OpenAILineParse parsed = OpenAIEventStream.ParseLine(step.Event.Data);
            if (parsed.Error != null) {
                resp.Dispose();
                yield return parsed.Error;
                yield break;
            }
            OpenAIResponse line = parsed.Line;
            if (line.Usage != null) {
                usageAcc = OpenAIEventStream.ToTokenUsage(line.Usage);
            }
            if (line.Choices.Count == 0) {
                continue;
            }
            OpenAIChoice choice = line.Choices[0];
            if (choice.Delta == null) {
                // 无 delta 块（可能仅携带 finish_reason）：终结由 [DONE]/EOF 驱动
                continue;
            }
            OpenAIDelta delta = choice.Delta;
            ...
            if (delta.ToolCalls != null) {
                ...
                while (i < delta.ToolCalls.Count) {
                    OpenAIToolCall tc = delta.ToolCalls[i];
                    int tcidx = tc.Index;
    ...
    private static async Task<OpenAIStreamStart> StartAsync(OpenAIOptions options, HttpClient http, string url, AIRequest request) {
        OpenAIStreamStart r = new OpenAIStreamStart();
        try {
            string body = OpenAIRequestBuilder.BuildRequestJson(request, options, true);
            ...
            if (resp.StatusCode != 200) {
                ...
                string errMsg = OpenAIResponseParser.ExtractErrorMsg(errBody, resp.StatusCode);
                resp.Dispose();
            ...
            r.Error = AIStreamEvent.Error("StreamError", "OpenAIChatClient.StreamEventsAsync: " + (ex != null && ex.Message != null ? ex.Message : "stream error"));
            return r;
        }
    }

    /// <summary>非抛错解析边界：JSON 文档 → OpenAIResponse DTO；解析失败转 Error 终结事件。</summary>
    private static OpenAILineParse ParseLine(string json) {
        OpenAILineParse r = new OpenAILineParse();
        try {
            r.Line = new OpenAIResponse();
...
internal class OpenAIStreamStart {
...
internal class OpenAILineParse {
    /// <summary>解析出的响应 DTO。</summary>
    public OpenAIResponse Line { get; set; }

    /// <summary>失败终结事件。</summary>
    public AIStreamEvent Error { get; set; }

    public OpenAILineParse() {
        this.Line = null;
        this.Error = null;
    }
}
