...
| **TabControl** | ✅ SelectedIndex + Children=`TabItem` + **内置页签栏 Header** | ✅ Background/SelectedIndex/TabCount/Header{i}/HeaderWidth{i}/HeaderBarHeight/**HeaderScrollOffset**/HeaderOverflow/OverflowArrowWidth + PointerRouter | ✅ 顶栏**按文案测宽左对齐** Header + **溢出 PushClip 水平滚** + **挤栏左右箭头 chrome** + 选中 Accent 底线 + LayoutShell 客户区（**未迁模板**：Panel 内容子树，`templated` 门禁不适用） | ✅ 页签栏点击切换 SelectedIndex（C HitTabIndex = strip localX+HeaderScrollOffset）；✅ 顶栏滚轮调偏移 + 选中入视；✅ **溢出箭头**（HitTabOverflow ±1 → TabOverflowScrollStep）；⛔ 切换动画 / 关闭按钮另排 | ⛔ 不适用 |
