...
>
> **核对增补（2026-09-11 · Tab 关闭按钮 · VSCode 悬停）**：① 选中页「x」常显；未选中仅页签栏悬停该页才显（`HoverTabIndex`；C `rt_ui_tabcontrol_update_hover` 与 HitTabIndex 同源，箭头/客户区清 -1）；② 槽位常预留（`TabCloseSlotWidth`/`TabCloseGlyphSize`，主题 `Text.Secondary`）—悬停不改测宽；③ `HitTabClose` 仅当「x」可见；箭头优先不抢 `HitTabOverflow`；④ `CloseTab` / `TabItem.Close`：选中关闭则右侧滑入同索引，末页选左；关选中左侧则 `SelectedIndex-1`；⑤ 关闭页 `IsClosed`，栏撤出、Arrange 视口外，**仍留 Children**（无 `rt_ui_element_remove_child` ABI）。headless `l2_ui_tab_close_batch`。⛔ 切换动画 / Tooltip。

> **核对增补（2026-09-11 · G2 最小硬门槛）**：`LivePreviewHost` LoadSpec（运行时 `ArmlParser`，不二次 typeck）→ 离屏单帧；`ApplyPatch` 字符串 DP（`DpValueConverter`）改即见；`CapturePng` / `GetLayoutSnapshot` / `Reset`。headless `l2_ui_live_preview_g2_batch`。绘制走 `TreeDrawListBuilder` 同 DrawList IR。**不**宣称 G1 双宿主像素 / G3 VideoSurface / 运行时 typeck / 交互预览。
>
> **核对增补（2026-09-10 · MessageBox 主题 Style 面板）**：① `Themes/Controls/MessageBox.arml` 短键 `Panel`（TargetType=Border）→ `Color.Surface`/`Color.Border`/`Size.Border.Thickness`/`Radius.Surface`/`Size.MessageBox.Padding`；② `BuildContent` 根改为 Border 包 StackPanel（禁裸 Stack 涂 Surface）；③ Open 前 `ApplyDialogStyles`（Popup 层不在 MainWindow `ApplyStyleTree`）刷显式键 → Primary/Default `AppliedStyleKeys`；④ 几何 `MessageBoxWidth`/`MessageBoxButtonWidth`/`SpacingLG` 内宽推算；⑤ 门禁扩 `control_metrics_owns_geometry`。**不**宣称 Path 矢量图标 / 多实例队列。
>
> **核对增补（2026-09-10 · Button CanExecuteChanged→IsEnabled）**：`ICommand.CanExecuteChanged`（`Signal<bool>`）+ `RelayCommand.RaiseCanExecuteChanged`；`Button` 赋 Command / 改 Parameter / 收脉冲时直写 `IsEnabled = CanExecute(param)`（route 槽 int 逃逸安全）；RaiseClick 仍门控 IsEnabled→Clicked→Execute；headless `l2_ui_button_command_batch`（含 `can_execute_changed_syncs`）。**不**宣称 ARML `{x:Bind}/{Binding}` Command、RoutedCommand、CommandManager.RequerySuggested、WPF IsEnabledCore 合取、完整 MVVM 全家桶。
>
> **核对增补（2026-09-10 · Tab 溢出左右箭头）**：挤栏时两端箭头 chrome + `HitTabOverflow` 步进（`TabOverflowScrollStep`）；条带/命中/渲染同源；ArmlDemo 10–12 无需改 demo。**不**宣称切换动画 / 关闭按钮。
>
...
| 面 | 示例 | 用户是否直接使用 |
|----|------|------------------|
| **公开面** | `Window` · `Button` · `CodeEditor` · `VisualHost` · `LivePreviewHost` · 布局面板 · `DrawList` | ✅ |
...
| **Button** | ✅ Content/Background/Foreground/FontSize/IsEnabled/IsMouseOver/IsPressed/Command/CommandParameter/Click（string）+ **Clicked（Signal）**；**RaiseClick→ICommand.Execute**（CanExecute 同步查询；`RelayCommand`）；**CanExecuteChanged→IsEnabled**（`RaiseCanExecuteChanged` / 赋 Command 立即同步）；变体 = **`Style="{StaticResource Primary, Small}"`**（§0.1.1；**禁 Class/Appearance/`*.Size.SM`**）；**无 Appearance DP**；**默认 Template**（PART_Chrome+PART_Content） | ✅ Content/FontSize/Background/Foreground/IsEnabled/StyleKeys/PaddingX\|Y + PointerRouter | ✅ **模板优先**（PART VSM）；无模板宿主回退 | 🟡 Click + 指针 hit + M-focus；✅ CanExecuteChanged→IsEnabled；⛔ ARML Command 绑定后置 | ⛔ 不适用 |
...
| **MessageBox** | ✅ **ShowAsync**(owner, text, caption, buttons[, image], ct) → `Task&lt;MessageBoxResult&gt;`；按钮 **OK / OKCancel / YesNo / YesNoCancel**；结果 OK/Cancel/Yes/No；**MessageBoxImage** None/Information/Warning/Error/Question | ✅ 经 **Popup** 附加层（`IsLightDismissEnabled=false`）；**主题 Style 面板**（Border `Style="Panel"` · `MessageBox.arml`）；Open 前 StyleManager；正文 `Color.Text.Primary`；OK/Yes=Primary / Cancel·No=Default（`AppliedStyleKeys`）；图标 = 主题色色块 + 符号 TextBlock | ✅ 复用 PopupLayer/Backdrop + **Border 面板**（Radius.Surface）+ Button/TextBlock/StackPanel 通用递归（无专属 wgpu 分支） | ✅ 按钮点击完成 TCS；Esc→OKCancel/YesNoCancel=Cancel / YesNo=No / OK-only=OK（KeyboardRouter）；蒙层不关；禁 Win32/原生对话框；同窗单实例（再开先 Cancel 前者）；⛔ Path 矢量图标 / 多实例队列另排 | ⛔ 不适用 |
| **TabControl** | ✅ SelectedIndex + Children=`TabItem` + **内置页签栏 Header** + **CloseTab** / **HoverTabIndex** | ✅ Background/SelectedIndex/TabCount/Header{i}/HeaderWidth{i}/HeaderBarHeight/**HeaderScrollOffset**/HeaderOverflow/OverflowArrowWidth/**CloseSlotWidth**/**HoverTabIndex** + PointerRouter（HitTabIndex/HitTabOverflow/**HitTabClose** + visual 离栏清悬停） | ✅ 顶栏**按文案测宽左对齐** Header + **溢出 PushClip 水平滚** + **挤栏左右箭头 chrome** + **关闭 x（选中常显 / 未选中悬停）** + 选中 Accent 底线 + LayoutShell 客户区（**未迁模板**：Panel 内容子树，`templated` 门禁不适用） | ✅ 页签栏点击切换 SelectedIndex（C HitTabIndex = strip localX+HeaderScrollOffset）；✅ 顶栏滚轮调偏移 + 选中入视；✅ **溢出箭头**（HitTabOverflow ±1 → TabOverflowScrollStep）；✅ **关闭按钮**（选中常显；未选中 HoverTabIndex；HitTabClose → CloseTab；箭头优先）；⛔ 切换动画 / Tooltip | ⛔ 不适用 |
| **TabItem** | ✅ Header + Children 内容树 + **IsClosed** / **Close** | ✅ Background/Header | ✅ **内容宿主**：Measure 有界时取视口宽高；Arrange 槽位=整页 `finalSize`（Stretch 拉满）；关闭页视口外 | 🟡 关闭经父 `CloseTab`（无独立页签栏 hit） | ⛔ 不适用 |
...
| **LivePreviewHost** | ✅ LoadSpec / ApplyPatch / ApplyPatches / CapturePng / GetLayoutSnapshot / Reset + ViewportW/H / RootElement（派生 VisualHost） | ⛔ 无 HWND / 无 PointerRouter（预览无交互） | ✅ 离屏 `WgpuRender` + `TreeDrawListBuilder`（同一 DrawList IR；**非** WindowHost `RenderTree` 第二套） | ⛔ 无输入；✅ **G2 最小硬门槛**（`l2_ui_live_preview_g2_batch`）；⛔ G1 双宿主像素 / G3 VideoSurface / 运行时 typeck | 🟡 预览区推荐虚拟化模板 |
...
| `ICommand` / `RelayCommand` | `std/UI/Components/ICommand.as` · `RelayCommand.as` | CanExecute/Execute + **CanExecuteChanged（Signal）**；`RelayCommand.RaiseCanExecuteChanged`；`Button` 订阅→IsEnabled（headless `ui_button_command`）；⛔ CommandManager.RequerySuggested / ARML Command 绑定后置 |
...
> **核对增补（2026-09-09 · Tab 溢出滚动最小面）**：① 镜像 `HeaderScrollOffset`；② RenderTree 顶栏 `PushClip` + `cursorX=lx-offset`；③ C `HitTabIndex` 用 `localX+offset` 与渲染同源；④ 顶栏滚轮（`scroll_win32` 认 TabControl 栏 + ScrollRouter→`ApplyHeaderWheelDelta`）；⑤ 选中切换 `EnsureSelectedTabVisible`；⑥ ArmlDemo 追加 10–12 长 Header 挤栏冒烟。

> **核对增补（2026-09-10 · Tab 溢出左右箭头）**：① `HeaderOverflow` + `OverflowArrowWidth` 镜像；② 条带宽 = 栏宽−2×`TabOverflowArrowWidth`（clamp / 选中入视 / PushClip / C 命中同源）；③ RenderTree 两端三角 chrome（端点 muted）；④ C `HitTabOverflow` ±1 → `ApplyOverflowArrow`（`TabOverflowScrollStep`）；⑤ ArmlDemo 10–12 挤栏无需改 demo。⛔ 切换动画 / 关闭按钮另排。
