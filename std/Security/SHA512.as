...
/// <summary>
/// SHA-512 hash facade（RFC 026 M3 · FIPS 180-4）。
/// 输入 byte[] → 64 字节摘要；ToHex 输出 lowercase hex（128 字符）。
/// 底层失败抛 CryptographicException（报错 > 静默）。
/// 可证伪：UnitTest/Arc/SecurityTests（NIST CAVP SHA-512ShortMsg 向量）。
/// </summary>
public class SHA512 {
    /// <summary>内部 ABI：byte[] → byte[64]；失败返回 null（公开体转为异常）。</summary>
    [Builtin(ABI = "rt_crypto_sha512_arr")]
    private static byte[] _ComputeHash(byte[] data) { return null; }

    /// <summary>计算 SHA-512 摘要（64 字节）。输入 null 抛 ArgumentNullException。</summary>
    public static byte[] ComputeHash(byte[] data) {
        ...
        if (digest == null) {
            throw new CryptographicException("SHA-512 computation failed.");
        }
        return digest;
    }
