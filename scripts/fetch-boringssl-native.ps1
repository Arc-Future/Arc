...
# crypto_native 共享库（含 M0 探针 shim + M1 `rt_crypto_*` ABI 实现面），产物落到
#   crates/runtime-crypto/bin/<windows|linux|macos>/
...
if ($IsWin) {
    $Platform = "windows"
    $SharedName = "crypto_native.dll"
} elseif ($IsMac) {
    $Platform = "macos"
    $SharedName = "libcrypto_native.dylib"
} else {
    $Platform = "linux"
    $SharedName = "libcrypto_native.so"
}
...
$VendorDir = Join-Path $Root "crates/runtime-crypto/bin/$Platform"
$DllTarget = Join-Path $VendorDir $SharedName

if (!$Force -and (Test-Path $DllTarget)) {
    Write-Host "$SharedName already present ($DllTarget); use -Force to re-build"
    exit 0
}
...
try {
    ...
    try {
        ...
        $Clang = if (Get-Command clang -ErrorAction SilentlyContinue) { "clang" } else { "clang.exe" }
        & $Clang @Cflags @Include $Sources 2> $CompileLog
    ...
    Write-Host "Linking $SharedName..."
    $Dll = Join-Path $BuildScratch $SharedName
    Push-Location $Obj
    try {
        ...
        $Clang = if (Get-Command clang -ErrorAction SilentlyContinue) { "clang" } else { "clang.exe" }
        if ($IsWin) {
            & $Clang -shared -O2 -o $Dll @($Objs) -lbcrypt -lcrypt32 2> $LinkLog
        } elseif ($IsMac) {
    ...
    if ($IsWin -and (Test-Path (Join-Path $VendorDir 'crypto_native.lib'))) {
